...
int ws_handle_task_get_page(struct ws_state *state, struct ws_message *request, cJSON *request_json) {   
    int ret;
    struct db_entry *entries[WS_MAX_PAGE_SIZE];

    cJSON *page_index_json = cJSON_GetObjectItemCaseSensitive(request_json, "page_index");
    cJSON *page_length_json = cJSON_GetObjectItemCaseSensitive(request_json, "page_length");

    if (!cJSON_IsNumber(page_index_json) || !cJSON_IsNumber(page_length_json)) {
        return ws_send_error(request, ret, "Missing value");
    }

    const short page_index = (page_index_json->valuedouble) >= 0 ? page_index_json->valuedouble : 0;
    const short page_length = (page_length_json->valuedouble) >= 1 ? ((page_length_json->valuedouble) <= WS_MAX_PAGE_SIZE ? page_length_json->valuedouble : WS_MAX_PAGE_SIZE) : 1;

    ret = db_get_page(state->config->db_state, entries, page_index, page_length);
    if (ret) {
    ...
    if (!cJSON_AddStringToObject(response_json, "task", "get_page")) {
        ret = -1;
        goto out;
    }
...
int ws_handle_task_claim_port(struct ws_state *state, struct ws_message *request, cJSON *request_json) {   
    int ret;
    struct db_entry *entries[WS_MAX_PAGE_SIZE];
    ...
    if (!cJSON_AddStringToObject(response_json, "task", "claim_port")) {
        ret = -1;
        goto out;
    }
...
int ws_handle_task(struct ws_state *state, struct ws_message *request, cJSON *request_json) {   
    ...
    if (!task_json->valuestring) {
        return ws_send_error(request, EINVAL, "Invald task string");
    }

    if (!strcmp(task_json->valuestring, "get_page")) {
        ret = ws_handle_task_get_page(state, request, request_json);
    } else if (!strcmp(task_json->valuestring, "claim_port")) {
        ret = ws_handle_task_claim_port(state, request, request_json);
    } else {
